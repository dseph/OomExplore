...
namespace OomExplore
{
    public partial class MessageForm : Form
    {
        ...
        public  MessageForm(ref Microsoft.Office.Interop.Outlook._Application oApp, ref Microsoft.Office.Interop.Outlook._NameSpace oNS, ref Outlook.MailItem oEmailMessage)
        {
            InitializeComponent();
          ...
          // New Message
        public MessageForm(ref Microsoft.Office.Interop.Outlook._Application oApp, ref Microsoft.Office.Interop.Outlook._NameSpace oNS)
        {
            InitializeComponent( );
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("25.1 _EmailMessage is null: " + (_EmailMessage == null).ToString());
                _EmailMessage = (Outlook.MailItem)_oApp.CreateItem(Outlook.OlItemType.olMailItem);
                System.Diagnostics.Debug.WriteLine("25.2 _EmailMessage is null: " + (_EmailMessage == null).ToString());
        ...
        private bool DisplayMessageInOutlook(bool bSaveCopy)
        {
            bool bRet = false;

            //if (_ResponseMessage == null)
            // {
            System.Diagnostics.Debug.WriteLine("40 _EmailMessage is null: " + (_EmailMessage == null).ToString());
            bRet = SetMessageFromForm(ref _EmailMessage);
            System.Diagnostics.Debug.WriteLine("41 _EmailMessage is null: " + (_EmailMessage == null).ToString());
            if (_ResponseMessage == null)
            {
                try
                {
                    if (bSaveCopy)
                    {
                        ((Outlook._MailItem)_EmailMessage).Display();
                        System.Diagnostics.Debug.WriteLine("42 _EmailMessage is null: " + (_EmailMessage == null).ToString());
                    }
                    else
                    {
                        ((Outlook._MailItem)_EmailMessage).Display();
                        System.Diagnostics.Debug.WriteLine("43 _EmailMessage is null: " + (_EmailMessage == null).ToString());
                    }
                    bRet = true;
                }
                catch (System.Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine("44 _EmailMessage is null: " + (_EmailMessage == null).ToString());
                    MessageBox.Show(ex.Message, "Error Sending Message");
                    bRet = false;
                }
            }
        ...
        private void btnOpenUsingOutlook_Click(object sender, EventArgs e)
        {

            System.Diagnostics.Debug.WriteLine("50 _EmailMessage is null: " + (_EmailMessage == null).ToString());
            _EmailMessage = (Outlook.MailItem)_oApp.CreateItem(Outlook.OlItemType.olMailItem);
            System.Diagnostics.Debug.WriteLine("51 _EmailMessage is null: " + (_EmailMessage == null).ToString());
            DisplayMessageInOutlook(true);
            System.Diagnostics.Debug.WriteLine("52 _EmailMessage is null: " + (_EmailMessage == null).ToString());

            // http://download.microsoft.com/download/5/D/D/5DD33FDF-91F5-496D-9884-0A0B0EE698BB/[MS-ASCMD].pdf
        }
